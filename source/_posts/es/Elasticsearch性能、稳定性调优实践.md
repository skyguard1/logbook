...
<p style=""><img src="/logbook/images/es/Elasticsearch性能、稳定性调优实践/4f05be2bbe65.jpg" alt="" style="position: relative; z-index: 2;" class="amplify"></p>
...
<pre><code>sed -i '/swap/s/^/#/' /etc/fstab
swapoff -a
</code></pre>
...
<pre><code>mount -o noatime,data=writeback,barrier=0,nobh /dev/sda /es_data
</code></pre>
...
<pre><code>indices.memory.index_buffer_size: 15%
...
<pre><code>cluster.routing.allocation.disk.include_relocations: false
</code></pre>
...
<pre><code># 1. 通过mapping指定 tags 字段为keyword类型
PUT my_index
{
  "mappings": {
    "my_type": {
...
<pre><code># 专有master节点的配置文件（conf/elasticsearch.yml）增加如下属性：
node.master: true
node.data: false
node.ingest: false
